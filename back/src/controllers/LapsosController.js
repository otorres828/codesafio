...
const borrar_lapso = (req, res) => {
  ...
  db.run(sql, [lapso_id], function (err) {
    ...
    res
      .status(200)
      .json({ id: this.lastID, mensaje: "Lapso borrado con éxito" });
  });
};
...
const actualizar_lapso = (req, res) => {
  ...
  const sql =
    "UPDATE lapsos SET nombre_lapso = ? WHERE lapso_id = ? AND usuario_id = ?";
  ...
  db.run(sql, [nombre_lapso, lapso_id, usuario_id], function (err) {
    ...
    res
      .status(200)
      .json({ id: this.lastID, mensaje: "Lapso actualizado con éxito" });
  });
};
